 ...
 *  (2) try 블록 실행ㅈ 중에 예외가 발생하면, 발생한 예외를 catch할 수 있는 catch 블록이 실행되고, finally 블록이 실행됨.
 *  (3) 발생한 예외를 catch할 수 있는 catch 블록이 없으면 프로그램은 비정상 종료.
 *  (4) catch 블록은 발생할 수 있는 예외 종류들에 따라서 여러개를 작성할 수 있음.
 *  (5) finally 블록은 선택(필수는 아님).
 *  
 *  다형성(polymorphism): SuperType vat = new SubType();
 ...
 *  catch 블록이 여러개일 때는 하위(sub) 타입의 예외들을 먼저 catch하고, 상위(super) 타입의 예외들은 나중에 catch해야 함. 
 */
